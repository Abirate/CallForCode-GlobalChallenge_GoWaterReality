...
3. The Code of GoWaterReality (JS,HTML,CSS,Node.JS), uses the [ArcGIS API for JavaScript](https://developers.arcgis.com/javascript/latest/), and the hosted Feature Layers on Arcgis Server Online(created in step 2), to develop many features: Editor Feature , Report issue Feature, Filter  feature, Find a component by attribute feature, consult any component on the network to have all the updated data aabout it,etc.
